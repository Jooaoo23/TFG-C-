...
void ventanaEmergentePuntos(int i) {
    ...
    // 1. LIMPIEZA INICIAL: Vital para evitar que la ventana se cierre sola
    // o procese clics que se hicieron en la mesa.
    while(kbhit()) getch(); 
    ...
    while(continuar) {
        // 2. DIBUJO
        redibujarFondoMesaContinental(); // Dibujamos el fondo (página activa)
        ...
        settextjustify(CENTER_TEXT, CENTER_TEXT); 
        settextstyle(DEFAULT_FONT, HORIZ_DIR, 2);
        outtextxy(750, 605, (char*)"ACEPTAR"); 
        ...
        // Dibujar texto actual y cursor
        outtextxy(280 + textwidth(bufferMensaje), 560, bufferEntrada);
        if ((long)time(NULL) % 2 == 0) // Un cursor que parpadea (opcional 2026 style)
            outtextxy(280 + textwidth(bufferMensaje) + textwidth(bufferEntrada), 560, (char*)"_");

        // 3. REFRESCO ÚNICO
        actualizarPantalla(); 

        // 4. LÓGICA DE TECLADO SEGURA
        if (kbhit()) {
            char tecla = getch();
            if (tecla == 0 || tecla == (char)224) { 
                getch(); // CONSUMIR segundo código de teclas especiales (flechas, etc.)
            } else if (tecla == 13) { // ENTER
                continuar = false; 
            } else if (tecla == 8 && pos > 0) { // BACKSPACE
                pos--; 
                bufferEntrada[pos] = '\0'; 
        ...
        delay(10); // Un pequeño respiro para el procesador
    }

    // 6. PROCESAMIENTO FINAL
    int ptsInput = 0;
    if (strlen(bufferEntrada) > 0) {
        // Evitar que un solo "-" rompa el sscanf
        if (strcmp(bufferEntrada, "-") == 0) ptsInput = 0;
        else sscanf(bufferEntrada, "%d", &ptsInput);
    }
    
    int m = miPartida.manoActual - 1;
    if (m < 0) m = 0; // Protección de índice

    miPartida.listaJugadores[i].puntosPorMano[m] = ptsInput;
    
    // Recalcular acumulados (esto es mejor hacerlo en una función aparte, pero aquí funciona)
    ...
    // LIMPIEZA FINAL: Para que el clic de "ACEPTAR" no se use en la mesa
    clearmouseclick(WM_LBUTTONDOWN);
    while(kbhit()) getch(); 
}
